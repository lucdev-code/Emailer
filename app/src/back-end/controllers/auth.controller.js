...
export const verifyEmailStudent = async (req, res) => {
    try {
        ...
        if(checkEmail.rows.length > 0) {
            ...
            if(checkEmail.rows[0].email_verified === true) return res.json({checkEmail, message: 'Se encontro el email!', status: 'OK', email_verified: true})
            else return res.json({checkEmail, message: 'Se encontro el email!', status: 'OK', email_verified: false})
        }
